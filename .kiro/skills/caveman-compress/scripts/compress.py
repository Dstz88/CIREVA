...
def backup_dir_for(filepath: Path) -> Path:
    ...
    `.original.md` copies as live files. Base dir is platform-aware:
      ...
      - else:    $XDG_DATA_Beranda/caveman-compress/backups if set,
    ...
    if os.name == "nt" or sys.platform == "win32":
        ...
        base = Path(local_appdata) if local_appdata else Path.Beranda() / "AppData" / "Local"
        base = base / "caveman-compress" / "backups"
    else:
        xdg = os.environ.get("XDG_DATA_Beranda")
        base = Path(xdg) if xdg else Path.Beranda() / ".local" / "share"
        base = base / "caveman-compress" / "backups"
